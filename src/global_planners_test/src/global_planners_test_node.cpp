// FONTE: https://answers.ros.org/question/377450/costmap2dros-object-is-not-correctly-created/

// outra fonte fonte: https://answers.ros.org/question/228253/generating-a-2d-costmap-from-a-mapyaml-file/

// FONTE: http://wiki.ros.org/navigation/Tutorials/Writing%20A%20Global%20Path%20Planner%20As%20Plugin%20in%20ROS
...
//   costmap_2d::Costmap2DROS willow_garage_costmap( "willowgarage_costmap", tf);
  ...
  while (ros::ok()){
    ...
    if (navfn.makePlan(start, goal, plan)){
      tam= plan.size();
      ROS_WARN("ACHOU PLANO DE %d", tam);
      // publica em /global_planners_test_node/my_navfn_planner/plan
    }else{
      ROS_WARN("NOPE");
    }
  }
