...
#include <central_control/central_control.h> /////////// FUNFA PARA COMPILAR, FALTA testar
...
// CONFIG_TESTES_REAIS:
#define NUM_ROBOTS 2//1//3//5

#define INITIAL_MAP 1  //////// 0 para publicar um mapa com todos os goals da forma

#define MAX_VEL 0.3
#define MIN_DIST 0.1 //entre goal e centro da formacao para terminar a operacao

#define INC_DIST 0.2/// tamanho do turtlebot
...
class Control{
  public:
    ...
    Control(float formation_side, float e[NUM_ROBOTS], float v[NUM_ROBOTS][2]){
      ...
      for(int it=0; it < NUM_ROBOTS; it++){
        pot_parameter_e[it] = e[it]; // e entre [-2, +2]
  ...
  private:
    ...
    float formation_side_size;// = 4.0;

    // float ang_intermediate_goal = 0.0; //em rad

    float pot_parameter_e [NUM_ROBOTS];// = {-0.4, -0.4, -0.4}; //, 0.25, 0.25};  // e entre [-2, +2]

    //  	Burger--> Maximum translational velocity = 0.22 m/s
    // Maximum rotational velocity =	2.84 rad/s (162.72 deg/s)

    // position of the robot in the formation
                                // x, y
    float pot_parameter_v [NUM_ROBOTS][2];// = //{{0.0,  1.0},
                                            //  {0.0, -1.0},
                                            //  {0.0, 0.0}}; ////////3
                                            //  {{0.0,  1.0},
                                            //  {0.0, -1.0},
...
void global_costmap_callback(const nav_msgs::OccupancyGrid& msg){
  ...
  void Control::InitializePotentialField(){
    ...
    // float ang = ComputeYaw(formation_center.orientation) + ang_intermediate_goal;

    // intermediate_goal.position.x = formation_center.position.x +  cos(ang) * 0.9*formation_side_size/2.0;
    // intermediate_goal.position.y = formation_center.position.y +  sin(ang) * 0.9*formation_side_size/2.0;
  ...
  void Control::AddNeighborAsObstacle(){
    // For kExtraExpansion
    //// previne de um robô tampar totalmente o goal intermediario
    float distance = 0.10;//1.0;//0.5; //robot raudius = 0.105 m
  ...
  void Control::UpdatePotentialField(int robot_num){
    ...
    local_v[0] = pot_parameter_v_current[robot_num-1][0] - (intermediate_goal.position.x - formation_center.position.x);//robot_on_formation[robot_num].pose.position.x;
    local_v[1] = pot_parameter_v_current[robot_num-1][1] - (intermediate_goal.position.y - formation_center.position.y);//robot_on_formation[robot_num].pose.position.y;
...
std::tuple<float, float> Control::GradientDescentOrientationMag(int robot_num){ // tambem calcula a magnetude do gradiente para tentativa de parar antes de estar em potetencial planificado
  int cell_i, cell_j;
...
void Control::ExpandObstacles(){
  float distance = 0.25;//1.0;//0.5; //robot raudius = 0.105 m
  ...
  for(int robot_num = INITIAL_MAP; robot_num <= NUM_ROBOTS; robot_num++)
    ...
    // for(int it=0; it<map_size; it++){
        if(global_map_[robot_num][it].CELL_TYPE == kObstacle){
          // std::tie(i_cell, j_cell)=vectorIndexToMatrixIndices(it);
          ...
          for(; i < global_map_width_ && i < i_cell + cell_distance; i++){
            ...
            for(; j < global_map_height_ && j < j_cell + cell_distance; j++){
              int map_index = matrixIndicesToVectorIndex(i,j);
              if(global_map_[robot_num][map_index].CELL_TYPE != kObstacle &&//<= 0 &&
                sqrt(pow(i-i_cell, 2) + pow(j-j_cell, 2)) < cell_distance){
                // global_map_[robot_num][map_index].ORIGINAL_MAP = 99;
...
void Control::FirstMoveFormation(navfn::NavfnROS *planner){
  ...
  if(plan.size() < 1){
      ROS_WARN("tam plan %ld, SEM caminho para x: %f   y: %f", plan.size(), goal.pose.position.x, goal.pose.position.y);
    }else{
      ROS_WARN("tam plan %ld, caminho para x: %f   y: %f", plan.size(), goal.pose.position.x, goal.pose.position.y);
    }
...
void Control::MoveFormation(navfn::NavfnROS *planner){
  ...
  for(int robot_num = INITIAL_MAP; robot_num <= NUM_ROBOTS; robot_num++){
    ...
    /////para se o robo se afasta da posição ideal
    ///////////robots_position[robot_num] e robot_on_formation[robot_num]
    ...
    ///////////para se o robô está no limite do mapa potencial
    float dist_robot_form_center;
    // robots_position[robot_num] e formation_center
  ...
  if(!stop){
    ...
    int prox_pose = 5; // para garantir que o prox pose eh diferente da pose atual do centro
    ...
    ROS_WARN("ANDANDO");// prox pose a %f", dist_prox_pose);
  }else{    
    ROS_WARN("FORM PARADA");
    #endif
  }
  time_ant = time_now;
...
void Control::CalculateVparamCurrent(){
  ...
  for(int robot_num = INITIAL_MAP; robot_num <= NUM_ROBOTS; robot_num++){
    // robot_on_formation[robot_num].pose.position.x = initial_pot_parameter_v_dist_ang[robot_num-1][0] * cos(initial_pot_parameter_v_dist_ang[robot_num-1][1] + formation_ang);
    // robot_on_formation[robot_num].pose.position.y = initial_pot_parameter_v_dist_ang[robot_num-1][0] * sin(initial_pot_parameter_v_dist_ang[robot_num-1][1] + formation_ang);
    ...
    for(test_dist = INC_DIST; test_dist <= initial_pot_parameter_v_dist_ang[robot_num-1][0] && !obst; test_dist = test_dist + INC_DIST){
      ...
      ////////fazer quadrado da tamanho do robo para verificar
      ...
      // int map_index = i_central + j_central * global_map_width_;
      // if(global_map_[robot_num][map_index].CELL_TYPE == kUnknown || global_map_[robot_num][map_index].CELL_TYPE == kObstacle || global_map_[robot_num][map_index].CELL_TYPE == kExtraExpansion){
      //   break;
      // }
    }
...
void Control::SetGoalPosition(float x, float y, float z, float ang){
  ...
  ROS_WARN("CENTRO FINAL x: %f   y: %f", goal.pose.position.x, goal.pose.position.y);
  ...
  ROS_WARN("goal FINAL x: %f   y: %f  ang utilizado: %f", goal.pose.position.x, goal.pose.position.y, formation_ang_final);
...
int main(int argc, char** argv){
  ...
  // float v[NUM_ROBOTS][2] = {{0.5,  1.0},
  //                           {-0.5,  1.0},
  //                           {0.0, -1.0}};

  // PARA UM ROBO
  // float e[NUM_ROBOTS] = {-0.4};
  // float v[NUM_ROBOTS][2] = {{0.0, 0.0}};

  // CONFIG_TESTES_REAIS: PARA DOIS ROBOS
  float e[NUM_ROBOTS] = {-0.4, -0.4};
  float v[NUM_ROBOTS][2] = {{0.0, -0.5}, {0.0, 0.5}};

  // float formation_side = 4.0;
  float formation_side = 3.0; //////////PARA TESTAR COM ROBOTS REAIS
  ...
  // Room
  // central_control.SetInitialStartPosition(-3, 0, 0 , 0);
  // central_control.SetGoalPosition( 2, 0, 0, 0);

  // small Room
  // formation_ang_initial = 0;///////////funciona com 3.14/2;
  // central_control.SetInitialStartPosition(0, 0, 0 , 0);

  // // Ampulheta
  // central_control.SetInitialStartPosition(1.0, -6.0, 0.0, 1.57);
  // // central_control.SetGoalPosition(-2.0, 3.0, 0.0, 1.57); /// sala de baixo
  // central_control.SetGoalPosition(2.0, 3.0, 0.0, 1.57); /// sala de cima

  // Lab233
  // central_control.SetInitialStartPosition(0.0, 0.0, 0.0, 0.0);
  // central_control.SetGoalPosition(0.2, 0.0, 0.0, 0.0);

  // CONFIG_TESTES_REAIS:
  //  // Auditorio
  central_control.SetInitialStartPosition(-1.23, 0.35, 0.0, 0.0);
  central_control.SetGoalPosition( 1.85, -0.99, 0.0, 0.0);
  // central_control.SetGoalPosition( -3.85, 6.04, 0.0, 0.0);  ///// funciona, so entala num lugar, precisa investigar
  ...
  ROS_WARN("central_control terminou de operar");
  return 0;
}
